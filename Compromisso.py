...
class Compromisso:
    def __init__(self, nome, data, ini, fim, local, obs=""):
        ...
        self.ini = datetime.datetime.strptime(data + " " + ini, '%d/%m/%Y %H:%M:%S')
        self.fim = datetime.datetime.strptime(data + " " + fim, '%d/%m/%Y %H:%M:%S')
    ...
    def __str__(self):
        return "\n" + self.nome + "\n" + self.status + "\n" + self.ini.strftime("%d/%m/%Y, %H:%M:%S") + "\n" + \
               self.fim.strftime("%d/%m/%Y, %H:%M:%S") + "\n" + self.local + "\n" + self.obs + "\n"
